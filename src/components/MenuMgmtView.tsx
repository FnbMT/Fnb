...
export const MenuMgmtView = ({ 
  ...
  const handleExport = () => {
    const exportData = menu.filter(item => !item.isInventory && item.type !== 'goods').map(item => {
      ...
      // Format add-ons as NAME:PRICE:QUANTITY
      const addOnsStr = item.addOns?.map(a => `${a.name}:${a.price}:${a.quantity || 1}`).join('|') || '';
    ...
    reader.onload = async (evt) => {
      ...
      for (const processed of processedItems) {
        ...
        if (addOnsStr && typeof addOnsStr === 'string') {
          ...
          parts.forEach(part => {
            const [nameOrCode, priceStr, qtyStr] = part.split(':');
            if (nameOrCode && priceStr) {
              const price = Number(priceStr.trim().replace(/[^0-9.]/g, ''));
              const qty = qtyStr ? Number(qtyStr.trim().replace(/[^0-9.]/g, '')) : 1;
              const inv = menu.find(m => 
                (m.isInventory || m.type === 'goods' || m.category === 'Nguyên liệu') &&
                (m.code.toLowerCase() === nameOrCode.trim().toLowerCase() || m.name.toLowerCase() === nameOrCode.trim().toLowerCase())
              );
              if (!isNaN(price)) {
                item.addOns!.push({
                  id: Math.random().toString(36).substr(2, 7),
                  inventoryItemId: inv ? inv.id : '',
                  name: inv ? inv.name : nameOrCode.trim(),
                  unit: inv ? inv.unit : 'cái',
                  quantity: qty || 1,
                  price: price,
                  costPrice: inv ? (inv.costPrice || 0) : 0
                });
              }
            }
          });
  ...
  const inventoryItems = useMemo(() => {
    return menu.filter(item => item.isInventory || item.type === 'goods' || item.category === 'Nguyên liệu');
  }, [menu]);

  const filtered = menu.filter(item => {
    const isInvItem = item.isInventory || item.type === 'goods';
    if (isInvItem) return false;
  ...
  const handleSave = () => {
    ...
    // Validate add-ons: strictly from inventory
    if (newItem.addOns && newItem.addOns.length > 0) {
      const invalidAddon = newItem.addOns.find(a => !a.inventoryItemId);
      if (invalidAddon) {
        alert('Món ăn kèm bắt buộc phải chọn từ Kho hàng. Vui lòng chọn mặt hàng cho tất cả món kèm!');
        return;
      }
    }
    ...
    if (editingItem) {
      ...
      // Auto-generate code for dishes
      const count = menu.filter(i => !i.isInventory && i.type !== 'goods').length + 1;
      const code = `MA${count.toString().padStart(3, '0')}`;
    ...
    setShowAdd(false);
    setEditingItem(null);
    setNewItem({ name: '', price: 0, costPrice: 0, category: categories[1] || 'Món chính', unit: 'Đĩa', type: 'dish', isInventory: false, status: 'available', stock: 0, recipe: [], addOns: [], image: '' });
  };
  ...
  const addAddOn = () => {
    if (inventoryItems.length === 0) {
      alert('Chưa có mặt hàng trong kho! Vui lòng vào mục "Kho hàng" để tạo mặt hàng (VD: Bánh mì, Trân châu, Trứng...) trước khi thêm món ăn kèm.');
      return;
    }
    ...
    setNewItem({ 
      ...
      addOns: [
        ...
        { 
          id: Math.random().toString(36).substr(2, 7), 
          inventoryItemId: firstInv ? firstInv.id : '',
          name: firstInv ? firstInv.name : '', 
          unit: firstInv ? firstInv.unit : 'cái',
          quantity: 1,
          price: 0, 
          costPrice: firstInv ? (firstInv.costPrice || 0) : 0 
        }
      ] 
    });
  };
    ...
    <div className="p-4 md:p-8 space-y-4 md:space-y-6 h-full overflow-y-auto">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center border-b border-black/10 dark:border-white/10 pb-4 gap-4 xl:gap-0">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Thực đơn</h2>
        </div>
        <div className="flex flex-wrap lg:flex-nowrap gap-2 md:gap-3 w-full xl:w-auto">
          ...
          >
            <Plus className="w-5 h-5 lg:w-4 lg:h-4" /> Thêm món mới
          </button>
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>NHÓM MÓN: <span className="text-emerald-600 dark:text-emerald-500 font-bold ml-1">{activeCategory}</span></span>
            </div>
              ...
              <div className="absolute top-full mt-2 left-0 w-full md:w-96 bg-white dark:bg-[#1a1b1e] border border-black/10 dark:border-white/10 rounded-xl shadow-xl z-40 p-2 flex flex-col gap-1 max-h-[60vh] overflow-y-auto custom-scrollbar">
                {categories.map((cat, idx) => (
                  <div key={cat} className="relative group/cat flex items-center">
                    ...
                    {idx > 0 && idx < categories.length - 1 && onUpdateCategories && (
                      <button 
          ...
          <div key={item.id} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl overflow-hidden flex flex-row md:flex-col group hover:border-emerald-500/30 transition-all h-28 md:h-auto relative">
            <div className="w-28 h-28 md:w-full md:aspect-video flex-shrink-0 bg-gray-100 dark:bg-gray-800 relative">
              ...
              <div className="absolute bottom-1 left-1 md:bottom-2 md:left-2">
                <button 
                  onClick={() => onUpdateItem({ ...item, status: item.status === 'available' ? 'out_of_stock' : 'available' })}
                  className={cn(
                    "px-1.5 py-0.5 md:px-2 md:py-1 rounded-md text-[8px] md:text-[10px] font-bold uppercase tracking-wider cursor-pointer transition-all active:scale-95",
                    item.status === 'available' ? "bg-emerald-500 text-black shadow-lg shadow-emerald-500/20 hover:bg-emerald-400" : "bg-rose-500 text-white shadow-lg shadow-rose-500/20 hover:bg-rose-400"
                  )}
                  title="Nhấn để Bật/Tắt hết hàng"
                >
            ...
            <div className="p-3 md:p-4 flex flex-col justify-between flex-1 min-w-0">
              <div className="pr-12 md:pr-0">
                ...
                <p className="text-[10px] md:text-xs text-gray-500">{item.category}</p>
              </div>
              ...
              {activeTab === 'general' && (
                <div className="space-y-4">
                  <div className="flex flex-col items-center gap-4 p-6 bg-black/5 dark:bg-white/5 border border-dashed border-black/10 dark:border-white/10 rounded-2xl">
                    <div className="w-40 h-24 bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden border border-black/10 dark:border-white/10 relative group">
                      <img 
                        src={newItem.image || getMenuItemImage({ name: newItem.name, category: newItem.category, id: editingItem?.id })} 
                        alt="Preview" 
                        className="w-full h-full object-cover" 
                      />
                      {!newItem.image && (
                        <div className="absolute bottom-1 right-1 bg-black/60 backdrop-blur-sm text-[9px] text-white px-1.5 py-0.5 rounded font-medium">
                          Ảnh tự động
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <label className="px-4 py-2 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-lg text-xs font-bold hover:bg-black/10 dark:hover:bg-white/10 transition-all cursor-pointer">
                        ...
                        {newItem.image ? 'Thay đổi ảnh' : 'Tải ảnh riêng'}
                      </label>
                      {newItem.image && (
                        <button
                          type="button"
                          onClick={() => setNewItem({ ...newItem, image: '' })}
                          className="px-3 py-2 border border-rose-500/20 text-rose-500 hover:bg-rose-500/10 rounded-lg text-xs font-semibold transition-all cursor-pointer"
                        >
                          Dùng ảnh tự động
                        </button>
                      )}
                    </div>
                    <p className="text-[10px] text-gray-500">Món sẽ tự động có ảnh món ăn hấp dẫn nếu không tải ảnh riêng.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Nhóm thực đơn</label>
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          className="p-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl text-emerald-600 dark:text-emerald-500 hover:bg-black/10 dark:hover:bg-white/10 cursor-pointer"
                        >
              ...
              {activeTab === 'addons' && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400 italic">Món ăn kèm bắt buộc liên kết với mặt hàng trong Kho hàng để kiểm soát số lượng tồn và trừ kho tự động khi bán.</p>
                    </div>
                    <button 
                      type="button"
                      onClick={addAddOn}
                      className="text-xs font-bold text-emerald-600 dark:text-emerald-500 flex items-center gap-1 hover:underline cursor-pointer self-start sm:self-auto"
                    >
                      <Plus className="w-3.5 h-3.5" /> Thêm món kèm
                    </button>
                  </div>

                  {inventoryItems.length === 0 ? (
                    <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-700 dark:text-amber-400 text-xs">
                      ⚠️ Chưa có mặt hàng nào trong kho. Vui lòng vào mục <strong>Kho hàng</strong> để tạo các mặt hàng (VD: Bánh mì, Bánh mướt, Trân châu...) trước khi chọn làm món ăn kèm.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {(!newItem.addOns || newItem.addOns.length === 0) && (
                        <div className="text-center py-8 text-gray-400 text-xs border border-dashed border-black/10 dark:border-white/10 rounded-xl">
                          Chưa có món ăn kèm nào cho món này. Nhấn "Thêm món kèm" để chọn từ kho hàng.
                        </div>
                      )}
                      {newItem.addOns?.map((addon, idx) => {
                        const selectedInv = inventoryItems.find(i => i.id === addon.inventoryItemId) || 
                          inventoryItems.find(i => i.name.toLowerCase().trim() === addon.name.toLowerCase().trim());
                        return (
                          <div key={addon.id || idx} className="flex flex-col md:flex-row gap-2.5 md:gap-3 md:items-end p-3 rounded-xl border border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02]">
                            <div className="flex-1 min-w-0">
                              <label className="text-[10px] text-gray-500 uppercase font-bold mb-1 block">
                                Món ăn kèm (Từ Kho hàng) <span className="text-rose-500">*</span>
                              </label>
                              <select
                                className="w-full bg-white dark:bg-[#1a1b1e] border border-black/10 dark:border-white/10 rounded-lg py-2 px-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-emerald-500"
                                value={addon.inventoryItemId || selectedInv?.id || ''}
                                onChange={(e) => {
                                  ...
                                  if (inv) {
                                    addOns[idx] = {
                                      ...
                                      quantity: addOns[idx].quantity || 1
                                    };
                                  } else {
                                    addOns[idx] = {
                                      ...addOns[idx],
                              ...
                              >
                                ...
                                {inventoryItems.map(inv => (
                                  <option key={inv.id} value={inv.id}>
                                    {inv.code} - {inv.name} (Tồn: {Number(inv.stock || 0).toLocaleString('vi-VN')} {inv.unit})
                                  </option>
                                ))}
                              </select>
                            </div>
                            ...
                            <div className="w-full md:w-32">
                              ...
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  step="any"
                                  min="0.01"
                                  className="w-full bg-white dark:bg-[#1a1b1e] border border-black/10 dark:border-white/10 rounded-lg py-2 px-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-emerald-500 text-right font-mono"
                                  value={addon.quantity ?? 1}
                                  onChange={(e) => {
                                    const val = parseFloat(e.target.value) || 0;
                                    const addOns = [...(newItem.addOns || [])];
                                    addOns[idx] = { ...addOns[idx], quantity: val };
                                    setNewItem({ ...newItem, addOns });
                                  }}
                                />
                                <span className="text-xs text-gray-500 font-medium whitespace-nowrap min-w-[32px]">
                                  {addon.unit || selectedInv?.unit || 'cái'}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="w-full md:w-36">
                              ...
                              <CurrencyInput 
                                className="w-full bg-white dark:bg-[#1a1b1e] border border-black/10 dark:border-white/10 rounded-lg py-2 px-3 text-sm font-mono font-bold text-emerald-600 dark:text-emerald-500 focus:outline-none focus:border-emerald-500 text-right"
                                value={addon.price}
                                onChange={(val) => {
                                  const addOns = [...(newItem.addOns || [])];
                                  addOns[idx] = { ...addOns[idx], price: Number(val) };
                                  setNewItem({...newItem, addOns});
                                }}
                              />
                            </div>
                            ...
                            <button 
                              ...
                              className="p-2 text-rose-600 dark:text-rose-500 hover:bg-rose-500/10 rounded-lg self-end md:self-auto cursor-pointer"
                              title="Xóa món kèm"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
