...
import { Settings, Store, Shield, Bell, Save, User, Lock, X, Printer, Layout, Wifi, Plus, Trash2, CheckCircle2, AlertCircle, Search, Bluetooth, Monitor, Smartphone, Info, MapPin, QrCode } from 'lucide-react';
...
export const SettingsView = ({ settings, onUpdateSettings, currentUser, onResetAllTables, onChangePassword, localPrinters, onUpdateLocalPrinters }: { 
        ...
        {isAdmin && (
          <div className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-3xl p-8 space-y-6">
            <div className="flex items-center gap-3 text-emerald-600 dark:text-emerald-500">
              ...
              <h4 className="font-bold uppercase text-xs tracking-wider">Thông tin thanh toán QR (Chuyển khoản)</h4>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 uppercase font-bold mb-1 block">Ngân Hàng</label>
                <input 
                  type="text" 
                  list="bank-list"
              ...
              <p className="text-xs text-gray-500 italic mt-2">Thông tin này sẽ được dùng để tạo mã QR thanh toán nhanh cho khách hàng khi nhân viên in hóa đơn hoặc bấm vào bàn.</p>
            </div>
          </div>
        )}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-500/20 text-emerald-600 dark:text-emerald-500 rounded-xl hover:bg-emerald-500/30 transition-all font-bold text-sm whitespace-nowrap"
                    >
