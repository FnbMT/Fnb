...
export const TaxReportView = ({ invoices, currentUser, onPrint }: {
  ...
  const [summaryViewMode, setSummaryViewMode] = useState<'quarter' | 'half' | 'year'>('quarter');
  ...
  // Calculate Quarter revenues
  const q1Revenue = useMemo(() => {
    return invoices.filter(inv => {
      const d = new Date(inv.date);
      return d.getFullYear() === selectedYear && d.getMonth() >= 0 && d.getMonth() <= 2;
    }).reduce((sum, inv) => sum + inv.total, 0);
  }, [invoices, selectedYear]);

  const q2Revenue = useMemo(() => {
    return invoices.filter(inv => {
      const d = new Date(inv.date);
      return d.getFullYear() === selectedYear && d.getMonth() >= 3 && d.getMonth() <= 5;
    }).reduce((sum, inv) => sum + inv.total, 0);
  }, [invoices, selectedYear]);

  const q3Revenue = useMemo(() => {
    return invoices.filter(inv => {
      const d = new Date(inv.date);
      return d.getFullYear() === selectedYear && d.getMonth() >= 6 && d.getMonth() <= 8;
    }).reduce((sum, inv) => sum + inv.total, 0);
  }, [invoices, selectedYear]);

  const q4Revenue = useMemo(() => {
    return invoices.filter(inv => {
      const d = new Date(inv.date);
      return d.getFullYear() === selectedYear && d.getMonth() >= 9 && d.getMonth() <= 11;
    }).reduce((sum, inv) => sum + inv.total, 0);
  }, [invoices, selectedYear]);
          ...
          <div className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
              <div className="flex items-center gap-3">
                <h3 className="font-bold text-lg">Tổng hợp doanh thu</h3>
                <select 
                  value={selectedYear} 
                  onChange={(e) => setSelectedYear(Number(e.target.value))}
                  className="bg-white dark:bg-[#151619] border border-black/10 dark:border-white/10 rounded-lg px-3 py-1 font-bold text-sm outline-none focus:border-emerald-500"
                >
                  {[...Array(5)].map((_, i) => {
              ...
              <div className="flex bg-black/5 dark:bg-white/5 p-1 rounded-xl">
                <button
                  onClick={() => setSummaryViewMode('quarter')}
                  className={cn("px-4 py-1.5 rounded-lg text-sm font-bold transition-all", summaryViewMode === 'quarter' ? "bg-white dark:bg-[#151619] shadow-sm text-emerald-600 dark:text-emerald-500" : "text-gray-500 hover:text-gray-900 dark:hover:text-white")}
                >
                  Theo Quý
                </button>
                <button
                  onClick={() => setSummaryViewMode('half')}
                  className={cn("px-4 py-1.5 rounded-lg text-sm font-bold transition-all", summaryViewMode === 'half' ? "bg-white dark:bg-[#151619] shadow-sm text-emerald-600 dark:text-emerald-500" : "text-gray-500 hover:text-gray-900 dark:hover:text-white")}
                >
                  6 Tháng
                </button>
                <button
                  onClick={() => setSummaryViewMode('year')}
                  className={cn("px-4 py-1.5 rounded-lg text-sm font-bold transition-all", summaryViewMode === 'year' ? "bg-white dark:bg-[#151619] shadow-sm text-emerald-600 dark:text-emerald-500" : "text-gray-500 hover:text-gray-900 dark:hover:text-white")}
                >
                  Cả Năm
                </button>
              </div>
            ...
            {summaryViewMode === 'quarter' && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-4 shadow-sm">
                  <p className="text-xs text-gray-500 font-bold uppercase mb-1">Quý 1</p>
                  <p className="text-lg font-bold text-emerald-600 dark:text-emerald-500">{q1Revenue.toLocaleString('vi-VN')}đ</p>
                </div>
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-4 shadow-sm">
                  <p className="text-xs text-gray-500 font-bold uppercase mb-1">Quý 2</p>
                  <p className="text-lg font-bold text-emerald-600 dark:text-emerald-500">{q2Revenue.toLocaleString('vi-VN')}đ</p>
                </div>
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-4 shadow-sm">
                  <p className="text-xs text-gray-500 font-bold uppercase mb-1">Quý 3</p>
                  <p className="text-lg font-bold text-emerald-600 dark:text-emerald-500">{q3Revenue.toLocaleString('vi-VN')}đ</p>
                </div>
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-4 shadow-sm">
                  <p className="text-xs text-gray-500 font-bold uppercase mb-1">Quý 4</p>
                  <p className="text-lg font-bold text-emerald-600 dark:text-emerald-500">{q4Revenue.toLocaleString('vi-VN')}đ</p>
                </div>
              </div>
            )}

            {summaryViewMode === 'half' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-5 shadow-sm">
                  <p className="text-sm text-gray-500 font-bold uppercase mb-2">6 Tháng Đầu Năm</p>
                  <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-500">{firstHalfRevenue.toLocaleString('vi-VN')}đ</p>
                </div>
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-5 shadow-sm">
                  <p className="text-sm text-gray-500 font-bold uppercase mb-2">6 Tháng Cuối Năm</p>
                  <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-500">{secondHalfRevenue.toLocaleString('vi-VN')}đ</p>
                </div>
              </div>
            )}

            {summaryViewMode === 'year' && (
              <div className="grid grid-cols-1 gap-4">
                <div className="bg-white dark:bg-[#151619] border border-black/5 dark:border-white/5 rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-500 font-bold uppercase mb-1">Tổng Doanh Thu Cả Năm {selectedYear}</p>
                    <p className="text-3xl font-bold text-blue-600 dark:text-blue-500">{totalYearRevenue.toLocaleString('vi-VN')}đ</p>
                  </div>
                  <div className="mt-4 md:mt-0 text-right">
                    <p className="text-xs text-gray-400 font-medium">Bao gồm toàn bộ hóa đơn đã hoàn thành</p>
                    <p className="text-xs text-gray-400 font-medium">trong năm {selectedYear}</p>
                  </div>
                </div>
              </div>
            )}
