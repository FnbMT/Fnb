...
export const AttendanceScanner = ({
  ...
  onClose: () => void;
  onCheckIn: (record: Partial<AttendanceRecord>) => Promise<any>;
  ...
  const processCheckIn = async () => {
    ...
    try {
      ...
      if (res?.type === 'check_out') {
        setScanMessage('Đã ghi nhận kết thúc ca (Check-out) thành công!');
      } else {
        setScanMessage('Đã ghi nhận chấm công vào ca thành công!');
      }
