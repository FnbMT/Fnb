...
export class PrinterService {
  ...
  static async printDirect(printer: PrinterConfig, data: any, systemSettings: any): Promise<boolean> {
       ...
       if (data.type === 'bill') {
           ...
           if (item.selectedAddOns && item.selectedAddOns.length > 0) {
               item.selectedAddOns.forEach((addon: any) => {
                   const priceNote = addon.price > 0 ? ` (+${addon.price.toLocaleString()}đ)` : '';
                   html += `<div style="font-size: 0.85em; padding-left: 10px;">+ ${addon.name}${priceNote}</div>`;
               });
           }
    ...
    if (data.type === 'bill' && data.total !== undefined) {
       if (data.note && template.showNote) {
         html += `<div style="margin-bottom: 10px; font-style: italic;">Ghi chú: ${data.note}</div>`;
       }
