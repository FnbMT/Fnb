...
const Sidebar = ({ activeView, setView, onLogout, currentUser, unresolvedShiftsCount, packages }: { 
  ...
  const allMenuItems = [
    ...
    { id: 'tax_report', label: 'Báo cáo bán hàng', icon: FileText }
  ];
          ...
          >
            ...
            {table.status === 'occupied' && (() => {
              ...
              if (hasReady) {
                return (
                  <div className="pt-1">
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500 text-white animate-pulse shadow-xs">
                      <CheckCheck className="w-3 h-3" /> Bếp đã xong!
                    </span>
                  </div>
                );
              } else if (hasCooking) {
                return (
                  <div className="pt-1">
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-sky-500/15 text-sky-600 dark:text-sky-400 border border-sky-500/30">
                      <Flame className="w-3 h-3" /> Bếp đang nấu
                    </span>
                  </div>
                );
              } else if (hasPending) {
                return (
                  <div className="pt-1">
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30">
                      <Clock className="w-3 h-3" /> Chờ bếp làm
                    </span>
                  </div>
                );
              }
...
const MenuOrdering = ({ 
  ...
  const handlePrintProvisional = () => {
    ...
    onPrint({
      ...
      note: activeTable.note ? `PHIẾU TẠM TÍNH (${activeTable.note})` : 'PHIẾU TẠM TÍNH',
      customerName,
      customerType
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-4">
      <motion.div 
        ...
        className="relative bg-white dark:bg-[#1a1b1e] w-full max-w-[96vw] xl:max-w-7xl 2xl:max-w-[1550px] h-full h-[100dvh] md:h-[94vh] md:max-h-[94vh] md:rounded-3xl overflow-hidden flex flex-col lg:flex-row border border-black/10 dark:border-white/10 shadow-2xl"
      >
        {/* Full-screen Processing Overlay during checkout or dish calling */}
        {isProcessing && (
          <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-xs flex flex-col items-center justify-center gap-3 text-white">
            <div className="w-10 h-10 border-3 border-emerald-500/30 border-t-emerald-500 rounded-full animate-spin" />
            <p className="text-sm font-bold tracking-wide">Đang hoàn tất thanh toán...</p>
          </div>
        )}

        {/* Main Header (Common for both mobile views) */}
        <div className="lg:hidden p-4 pt-[max(env(safe-area-inset-top),16px)] border-b border-black/10 dark:border-white/10 flex flex-col gap-3 bg-white dark:bg-[#1a1b1e] shrink-0">
        ...
        {/* Mobile Toggle */}
        <div className="lg:hidden flex border-b border-black/10 dark:border-white/10 bg-white dark:bg-[#151619] shrink-0">
          <button 
        ...
        {/* Left: Menu Selection */}
        <div className={cn(
          "min-w-0 flex flex-col border-r border-black/10 dark:border-white/10 min-h-0 overflow-hidden lg:flex lg:flex-1 lg:h-full",
          viewMode === 'menu' ? "flex flex-1" : "hidden lg:flex"
        )}>
          {/* Header (Desktop only) */}
          <div className="hidden lg:block px-4 py-3 xl:px-6 xl:py-4 border-b border-black/5 dark:border-white/5 shrink-0">
            <div className="flex justify-between items-center gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <h2 className="text-xl xl:text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2 truncate">
                  {activeTable.name}
                  <span className="text-sm font-normal text-gray-500 shrink-0">Thêm món</span>
                </h2>
                <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 border border-emerald-500/20 uppercase font-bold shrink-0">
                  {activeTable.status === 'empty' ? 'Bàn trống' : 'Đang dùng'}
                </span>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                 ...
                 {!isOrderOnly && (
                   <>
                     <button 
                      onClick={() => setShowTransferModal(true)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-purple-500/10 border border-purple-500 text-purple-500 hover:bg-purple-500/20 transition-all cursor-pointer"
                    >
                     ...
                     <button 
                      onClick={() => setShowMergeModal(true)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-blue-500/10 border border-blue-500 text-blue-600 dark:text-blue-500 hover:bg-blue-500/20 transition-all cursor-pointer"
                    >
                ...
                <button 
                  onClick={() => setViewMode(viewMode === 'order' ? 'menu' : 'order')}
                  className={cn(
                  ...
                  title="Mở / Xem đơn hàng và thanh toán"
                >
                  ...
                  {(currentOrder.length > 0 || committedOrders.length > 0) && (
                    <span className={cn(
                      ...
                      viewMode === 'order' ? "bg-white text-emerald-700" : "bg-emerald-500 text-white"
                    )}>
                      {currentOrder.length + committedOrders.length}
                    </span>
                  )}
                ...
                >
                  ...
                  <span>{isPrintKitchenEnabled ? 'In bếp: BẬT' : 'In bếp: TẮT'}</span>
                </button>
                <button onClick={onClose} className="p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-full text-gray-600 dark:text-gray-400 cursor-pointer" title="Đóng">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
          ...
          <div className="p-3 md:p-4 border-b border-black/5 dark:border-white/5 shrink-0">
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
              <div className="relative w-full sm:w-56 md:w-64 shrink-0">
                ...
                <input 
                  ...
                  className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl py-2 pl-9 pr-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-emerald-500/50 transition-all"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
              <div className="flex-1 min-w-0 flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
                {categories.map(cat => (
                  <button
                    key={cat}
                    onClick={() => setActiveCategory(cat)}
                    className={cn(
                      "px-3 py-1.5 rounded-xl text-xs md:text-sm whitespace-nowrap transition-all cursor-pointer shrink-0",
          ...
          {/* Menu List/Grid */}
          <div className="flex-1 overflow-y-auto p-3 md:p-4 lg:p-5 custom-scrollbar pb-32 lg:pb-6 relative min-h-0 touch-pan-y">
            {isStrictCashier ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 md:gap-4">
                {filteredMenu.map(item => (
                  <button
                    ...
                    className="group bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-2.5 md:p-3 text-left hover:border-emerald-500/30 transition-all cursor-pointer flex flex-col items-start gap-2 h-full"
                  >
                    <div className="w-full aspect-video bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden shrink-0">
                      <img 
                    ...
                    <div className="w-full flex-1 flex flex-col justify-between">
                      <p className="font-bold text-gray-900 dark:text-white text-xs md:text-sm line-clamp-2 leading-snug">{item.name}</p>
                      <p className="text-emerald-600 dark:text-emerald-500 font-mono font-bold text-xs md:text-sm mt-1">{item.price.toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 1 })}đ</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[92%]">
              {currentOrder.length > 0 ? (
                <button 
                  onClick={() => setViewMode('order')}
                  className="w-full bg-emerald-600 hover:bg-emerald-500 active:scale-[0.98] text-white font-bold py-3.5 px-4 rounded-2xl shadow-[0_8px_30px_rgb(16,185,129,0.35)] flex items-center justify-between gap-3 transition-all"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm font-bold tracking-tight">
                      {activeTable.status === 'empty' ? 'Báo bếp & Mở bàn' : 'Xem đơn & Báo bếp'}
                    </span>
                  </div>
                  <span className="bg-black/20 text-white px-2.5 py-1 rounded-lg text-xs font-mono font-bold">
                ...
                <button 
                  onClick={() => setViewMode('order')}
                  className="w-full bg-black/80 dark:bg-white/10 border border-black/20 dark:border-white/20 text-white font-bold py-3.5 px-4 rounded-2xl backdrop-blur-md flex items-center justify-between gap-3 shadow-lg active:scale-[0.98] transition-all"
                >
                  <div className="flex items-center gap-2">
                    <ShoppingCart className="w-5 h-5 text-emerald-400" />
                    <span className="text-sm font-bold">Xem đơn & Thanh toán</span>
                  </div>
                  <span className="bg-emerald-500 text-white px-2.5 py-1 rounded-lg text-xs font-mono font-bold">
                ...
                <button 
                  onClick={() => setViewMode('order')}
                  className="w-full bg-black/80 dark:bg-white/10 border border-black/20 dark:border-white/20 text-white font-bold py-3.5 px-4 rounded-2xl backdrop-blur-md flex items-center justify-between gap-3 shadow-lg active:scale-[0.98] transition-all cursor-pointer"
                >
                  <div className="flex items-center gap-2">
                    <ShoppingCart className="w-5 h-5 text-emerald-400" />
                    <span className="text-sm font-bold">Xem đơn & Thanh toán</span>
        ...
        <div className={cn(
          "w-full lg:w-[400px] xl:w-[440px] 2xl:w-[480px] shrink-0 bg-white dark:bg-[#151619] flex flex-col min-h-0 min-w-0 overflow-hidden lg:flex lg:h-full",
          viewMode === 'order' ? "flex flex-1 min-h-0" : "hidden lg:flex"
        )}>
          <div className="p-3 md:p-4 border-b border-black/5 dark:border-white/5 flex justify-between items-center bg-white dark:bg-[#1a1b1e] lg:bg-transparent shrink-0">
            <div className="flex items-center gap-2">
              <button 
                onClick={() => setViewMode('menu')}
                className="lg:hidden flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-black/5 dark:bg-white/5 text-gray-700 dark:text-gray-300 border border-black/10 dark:border-white/10 hover:bg-black/10 transition-colors"
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={onClose} className="p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-full text-gray-600 dark:text-gray-400 cursor-pointer hidden lg:flex" title="Đóng">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-2 md:p-4 space-y-3 custom-scrollbar min-h-0">
          ...
          {/* Bottom Checkout & Action Area: Scrollable details + Pinned action buttons to avoid overflow */}
          <div className="shrink-0 border-t border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] flex flex-col min-h-0">
            <div className="max-h-[32vh] overflow-y-auto custom-scrollbar p-3 space-y-2.5">
              <div className="space-y-1.5 text-xs">
                {/* ... (Totals - Updated to reflect selection) ... */}
                <div className="flex justify-between items-center text-gray-600 dark:text-gray-400">
                  <span>Tạm tính ({selectedOrdersToPay.length > 0 ? `${selectedOrdersToPay.length} đơn đã chọn` : 'Tất cả đơn'})</span>
                  <span className="font-mono font-bold text-gray-900 dark:text-white">{finalSubtotal.toLocaleString()}đ</span>
                </div>
                {/* ... Discount/VAT inputs ... */}
                <div className="flex justify-between items-center text-gray-600 dark:text-gray-400">
                  <div className="flex items-center gap-2">
                ...
                <div className="pt-2 border-t border-black/5 dark:border-white/5 flex justify-between items-center text-gray-900 dark:text-white text-lg font-bold">
                  <span>Tổng cộng</span>
                  <span className="text-emerald-600 dark:text-emerald-500 font-mono text-xl">{finalTotal.toLocaleString()}đ</span>
                </div>
              </div>
                ...
                <div className="space-y-2.5 pt-1 border-t border-black/5 dark:border-white/5">
                  <div className="space-y-1">
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold flex justify-between items-center">
                      ...
                      {selectedCustomerId && (
                        <button onClick={() => setSelectedCustomerId('')} className="text-[10px] text-emerald-600 dark:text-emerald-400 hover:underline cursor-pointer">Bỏ chọn</button>
                      )}
                    </p>
                    <div className="relative">
                      {!showCustomerDropdown && !selectedCustomerId ? (
                        <button 
                          ...
                          className="w-full bg-gray-200 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-gray-600 dark:text-gray-400 text-left hover:border-emerald-500/50 cursor-pointer"
                        >
                        ...
                        <div 
                          onClick={() => setShowCustomerDropdown(true)}
                          className="w-full bg-gray-200 dark:bg-black/40 border border-emerald-500/50 rounded-lg px-2.5 py-1.5 text-xs text-emerald-600 dark:text-emerald-500 text-left cursor-pointer flex justify-between"
                        >
                          <span>
                        ...
                        <div className="relative">
                          <input
                            autoFocus
                            type="text"
                            className="w-full bg-gray-200 dark:bg-black/40 border border-emerald-500/50 rounded-lg px-2.5 py-1.5 text-xs text-gray-900 dark:text-white focus:outline-none"
                            placeholder="Nhập tên hoặc SĐT..."
                            value={customerSearch}
                            onChange={(e) => setCustomerSearch(e.target.value)}
                            onBlur={() => setTimeout(() => setShowCustomerDropdown(false), 200)}
                          />
                          <div className="absolute bottom-full left-0 right-0 mb-1 bg-white dark:bg-[#1a1b1e] border border-black/10 dark:border-white/10 rounded-lg max-h-48 overflow-y-auto z-50 shadow-2xl">
                            <button
                              className="w-full text-left px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5 text-xs text-gray-600 dark:text-gray-400 cursor-pointer"
                            ...
                            {filteredCustomers.map(c => (
                              <button
                                key={c.id}
                                className="w-full text-left px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5 text-xs text-gray-900 dark:text-white cursor-pointer"
                  ...
                  {systemSettings.bankCode && systemSettings.bankAccountNumber && finalTotal > 0 && paymentMethod === 'transfer' && (
                    <div className="p-2.5 bg-white dark:bg-black/20 rounded-xl border border-emerald-500/30 flex justify-between items-center gap-2.5">
                      <div className="flex-1 space-y-0.5">
                        <p className="text-[10px] font-bold uppercase text-emerald-600 dark:text-emerald-500 tracking-wider">Mã VietQR</p>
                        <p className="text-[9px] text-gray-500 uppercase leading-tight truncate">{systemSettings.bankAccountName} - {systemSettings.bankAccountNumber}</p>
                        <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-semibold cursor-pointer hover:underline block" onClick={() => setShowZoomedQR(true)}>Bấm để phóng to mã QR</span>
                      </div>
                      <div className="bg-white p-1 rounded-lg cursor-pointer shrink-0 shadow-sm border border-emerald-500/20" onClick={() => setShowZoomedQR(true)} title="Bấm để phóng to mã QR">
                        <img 
            ...
            {/* Pinned Action Buttons: Always visible at bottom, never overflow */}
            <div className="p-3 pb-[max(env(safe-area-inset-bottom),14px)] bg-white dark:bg-[#151619] border-t border-black/10 dark:border-white/10 shrink-0 space-y-2 shadow-[0_-4px_20px_rgba(0,0,0,0.15)]">
              {/* Buttons */}
              {currentOrder.length > 0 && (
                <button 
                  ...
                  className={cn(
                    "w-full py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:scale-[0.98] text-white font-bold shadow-lg shadow-emerald-600/30 cursor-pointer flex items-center justify-center gap-2 text-sm transition-all",
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      ...
                      {activeTable.status === 'empty' ? 'BÁO BẾP & MỞ BÀN' : (selectedOrdersToPay.length === 1 ? 'THÊM MÓN VÀO ĐƠN & GỬI BẾP' : 'TẠO ĐƠN MỚI & GỬI BẾP')}
                    </>
                  )}
                </button>
              )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <> <Printer className="w-3.5 h-3.5" /> THU & IN </>
                    )}
                  </button>
                  <button 
          ...
          {showZoomedQR && (
            ...
            >
              <motion.div 
                ...
                animate={{ scale: 1, y: 0 }}
                exit={{ scale: 0.9, y: 20 }}
                className="bg-white rounded-3xl p-6 max-w-sm w-full mx-auto relative flex flex-col items-center shadow-2xl"
              ...
              >
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-4 text-center">Quét mã để thanh toán</h3>
                <div className="bg-white p-2 rounded-2xl border border-gray-200 shadow-inner mb-4 w-full aspect-square flex items-center justify-center">
                  <img 
                    src={`https://img.vietqr.io/image/${systemSettings.bankCode}-${systemSettings.bankAccountNumber}-compact2.png?amount=${finalTotal}&accountName=${encodeURIComponent(systemSettings.bankAccountName || '')}&addInfo=${encodeURIComponent((systemSettings.bankCode?.toUpperCase() === 'ICB' || systemSettings.bankCode?.toUpperCase() === 'CTG' || systemSettings.bankCode?.toLowerCase()?.includes('vietin') ? 'SEVQR ' : '') + currentUser?.store?.code + ' THANH TOAN ' + activeTable.name)}`} 
                    alt="VietQR Zoomed" 
                ...
                <div className="text-center space-y-1">
                  <p className="font-bold text-gray-900">{systemSettings.bankAccountName}</p>
                  <p className="text-sm text-gray-500">{systemSettings.bankAccountNumber}</p>
                  <p className="text-xl font-black text-emerald-600 mt-2">{finalTotal.toLocaleString()} đ</p>
                </div>
              </motion.div>
            </motion.div>
          )}
          ...
          {configuringItem && (
            ...
            >
              <motion.div 
                initial={{ scale: 0.9, y: 20 }}
                animate={{ scale: 1, y: 0 }}
                className="bg-white dark:bg-[#1a1b1e] w-full max-w-md rounded-2xl md:rounded-3xl p-5 md:p-6 border border-black/10 dark:border-white/10 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
              >
                <div className="flex justify-between items-center shrink-0 pb-3 border-b border-black/5 dark:border-white/5">
                  <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate pr-2">{configuringItem.name}</h3>
                  <button onClick={() => setConfiguringItem(null)} className="text-gray-500 hover:text-gray-900 dark:hover:text-white p-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/5"><X className="w-5 h-5" /></button>
                </div>

                <div className="flex-1 overflow-y-auto custom-scrollbar py-3 space-y-4 min-h-0 pr-1">
                  {/* Main Quantity */}
                  <div className="flex justify-between items-center bg-black/5 dark:bg-white/5 p-3 rounded-xl">
                    <span className="text-gray-600 dark:text-gray-400 text-sm">Số lượng món chính</span>
                    <div className="flex items-center gap-3">
                      <button onClick={() => setItemQuantity(Math.max(0, itemQuantity - 1))} className="p-1.5 bg-black/10 dark:bg-white/10 rounded-lg hover:bg-white/20"><Minus className="w-3.5 h-3.5" /></button>
                      <span className="text-lg font-bold font-mono w-8 text-center">{itemQuantity}</span>
                      <button onClick={() => setItemQuantity(itemQuantity + 1)} className="p-1.5 bg-black/10 dark:bg-white/10 rounded-lg hover:bg-white/20"><Plus className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                  ...
                  {configuringItem.addOns && configuringItem.addOns.length > 0 && (
                    <div className="space-y-2.5">
                      <div className="flex justify-between items-center">
                        <p className="text-xs font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Món ăn kèm</p>
                        <span className="text-[11px] text-gray-400">Chọn thêm theo suất</span>
                      </div>
                      {configuringItem.addOns.map(addon => {
                        const count = selectedAddOns.filter(a => a.id === addon.id).length;
                        const invItem = menu.find(m => 
                          (m.isInventory || m.type === 'goods' || m.category === 'Nguyên liệu') &&
                          (m.id === addon.inventoryItemId || m.name.toLowerCase().trim() === addon.name.toLowerCase().trim())
                        );
                        const currentStock = invItem ? invItem.stock : undefined;
                        const isOutOfStock = invItem && invItem.trackStock !== false && currentStock !== undefined && currentStock <= 0;

                        return (
                          <div key={addon.id} className="flex justify-between items-center p-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5">
                            <div className="flex-1 pr-2">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <p className="font-medium text-gray-900 dark:text-white text-xs">{addon.name}</p>
                                {invItem && (
                                  <span className={cn(
                                    "text-[9px] font-mono px-1.5 py-0.5 rounded",
                                    isOutOfStock ? "bg-rose-500/10 text-rose-500 font-bold" : "bg-black/10 dark:bg-white/10 text-gray-500 dark:text-gray-400"
                                  )}>
                                    {isOutOfStock ? 'Hết hàng' : `Còn: ${invItem.stock} ${invItem.unit}`}
                                  </span>
                                )}
                              </div>
                              <p className="text-xs font-bold text-emerald-600 dark:text-emerald-500 mt-0.5">
                                {addon.price > 0 ? `+${addon.price.toLocaleString()}đ` : 'Miễn phí'}
                                {addon.quantity && addon.quantity !== 1 && (
                                  <span className="text-gray-400 font-normal ml-1">({addon.quantity} {addon.unit || invItem?.unit || 'phần'})</span>
                                )}
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                               {count > 0 && (
                                 <button 
                                   ...
                                   className="p-1 bg-black/10 dark:bg-white/10 rounded hover:bg-white/20 cursor-pointer"
                                 >
                                   <Minus className="w-3 h-3" />
                                 </button>
                               )}
                               <span className={cn("text-xs font-bold w-4 text-center font-mono", count > 0 ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-gray-600 dark:text-gray-400")}>{count}</span>
                               <button 
                                 type="button"
                                 disabled={isOutOfStock}
                                 onClick={() => setSelectedAddOns([...selectedAddOns, { ...addon, inventoryItemId: addon.inventoryItemId || invItem?.id }])} 
                                 className={cn(
                                   "p-1 rounded cursor-pointer transition-all",
                                   isOutOfStock ? "bg-black/5 dark:bg-white/5 text-gray-400 cursor-not-allowed opacity-50" : "bg-black/10 dark:bg-white/10 hover:bg-white/20"
                                 )}
                                 title={isOutOfStock ? 'Hết kho' : 'Thêm 1'}
                               >
                  ...
                  {/* Note */}
                  <div className="space-y-1.5">
                    <p className="text-xs font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Ghi chú</p>
                    <textarea 
                      className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl p-2.5 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-emerald-500"
                      rows={2}
                  ...
                  {/* Total price preview for configured dish + add-ons */}
                  <div className="flex justify-between items-center py-2 px-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                    <div>
                      <span className="text-xs text-emerald-800 dark:text-emerald-300 font-semibold block">Tạm tính</span>
                      <span className="text-[10px] text-gray-500">
                        ({configuringItem.price.toLocaleString()}đ {selectedAddOns.length > 0 ? `+ ${selectedAddOns.reduce((s, a) => s + a.price, 0).toLocaleString()}đ kèm` : ''}) × {itemQuantity}
                      </span>
                    </div>
                    <span className="text-sm md:text-base font-bold font-mono text-emerald-600 dark:text-emerald-400">
                      {((configuringItem.price + selectedAddOns.reduce((s, a) => s + a.price, 0)) * itemQuantity).toLocaleString()}đ
                    </span>
                  </div>
                </div>

                <div className="flex gap-2.5 pt-3 border-t border-black/5 dark:border-white/5 shrink-0">
                  <button 
                    onClick={() => setConfiguringItem(null)}
                    className="flex-1 py-2.5 rounded-xl border border-black/10 dark:border-white/10 text-gray-600 dark:text-gray-400 font-bold hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer text-xs"
                  >
                    Hủy
                  </button>
                  <button 
                    onClick={handleConfirmAddItem}
                    className="flex-1 py-2.5 rounded-xl bg-emerald-500 text-white font-bold hover:bg-emerald-400 cursor-pointer text-xs transition-colors shadow-sm"
                  >
                    Thêm vào đơn
                  </button>
                </div>
...
export default function App() {
  ...
  const [isRegistering, setIsRegistering] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      return params.get('register') === 'true' || params.get('mode') === 'register' || params.get('view') === 'register';
    }
    return false;
  });
  ...
  const triggerPrint = async (data: {
                    ...
                    ${item.selectedAddOns && item.selectedAddOns.length > 0 ? `
                      <div style="font-size: 0.8em; padding-left: 10px;">
                        ${Object.entries(item.selectedAddOns.reduce((acc, a) => {
                          const key = a.price > 0 ? `${a.name} (+${a.price.toLocaleString()}đ)` : a.name;
                          acc[key] = (acc[key] || 0) + 1;
                          return acc;
                        }, {} as Record<string, number>)).map(([text, count]) => `+ ${text} ${count > 1 ? `(x${count})` : ''}`).join('<br/>')}
                      </div>
                    ` : ''}
                  </td>
    ...
    if (data.type === 'bill' && data.total !== undefined) {
      content += `
          <div class="total-section">
            ${data.note && template.showNote ? `<div style="font-style: italic; font-size: 0.9em; margin-bottom: 10px; border-bottom: 1px dashed #eee; padding-bottom: 5px;">Ghi chú: ${data.note}</div>` : ''}
            <div class="row">
              <span>Tạm tính:</span>
    ...
    // Always update in-app printData
    setPrintData(data);

    // 4. Handle Silent Printing if enabled (Only on Native Android/iOS via TCP Socket)
    const isNative = Capacitor.isNativePlatform();
    if (isNative && systemSettings.silentPrinting) {
    ...
    // 5. Browser Print (Web / Desktop browser)
    // Uses hidden iframe to prevent popup blocker blocking window.open in browsers
    const printFrameId = '__app_web_print_iframe__';
    let printFrame = document.getElementById(printFrameId) as HTMLIFrameElement | null;
    if (!printFrame) {
    ...
    try {
      ...
      console.warn('Print iframe error, calling window.print:', err);
      window.print();
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubTables = onSnapshot(tablesQuery, (snapshot) => {
      // Clean up lingering notes ONLY on empty tables (which have no active customer)
      snapshot.docs.forEach(docSnap => {
        const data = docSnap.data();
        if (data.status === 'empty' && data.note) {
          updateDoc(doc(db, 'tables', docSnap.id), { note: deleteField() }).catch(console.error);
        }
      });
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          console.warn("Could not delete user document directly:", e);
        }
      }

      // 2. If Admin, remove store document
      if (currentUser.role === 'admin' && storeId) {
        try {
          await deleteDoc(doc(db, 'stores', storeId));
        } catch (e) {
          console.warn("Could not delete store document directly:", e);
        }
      }

      // 3. Clear local cache and session
      safeStorage.removeItem('fnb_master_user');
      localStorage.clear();
      sessionStorage.clear();
      alert('Tài khoản và dữ liệu của bạn đã được xóa hoàn toàn khỏi hệ thống.');
      window.location.reload();
    } catch (error: any) {
      console.error('Error deleting account:', error);
      alert('Lỗi khi xóa tài khoản: ' + (error.message || 'Vui lòng thử lại sau.'));
    }
  };
  ...
  const handleUpdateTableOrders = async (tableId: string, updatedOrders: OrderSession[]) => {
    try {
      setTables(prev => prev.map(t => t.id === tableId ? { ...t, orders: updatedOrders } : t));
      await updateDoc(doc(db, 'tables', tableId), {
        orders: updatedOrders
      });
    } catch (error) {
      console.error('Error updating table orders from kitchen:', error);
    }
  };
  ...
  const handleMergeTables = async (targetTableId: string, sourceTableIds: string[]) => {
    ...
    for (const sourceId of sourceTableIds) {
      ...
      if (sourceTable && sourceTable.orders) {
        newOrders = [...newOrders, ...sourceTable.orders];
        sourceNames.push(sourceTable.name);

        // Clear source table completely
    ...
    const mergeText = sourceNames.length > 0 ? `Gộp từ ${sourceNames.join(', ')}` : '';
    const updatedNote = targetTable.note 
      ? `${targetTable.note}; ${mergeText}` 
      : mergeText;

    await updateDoc(doc(db, 'tables', targetTableId), {
      orders: newOrders,
      status: 'occupied',
      note: updatedNote || deleteField()
    });
  };
  ...
  const handleTransferTable = async (sourceTableId: string, targetTableId: string) => {
    ...
    const transferText = `Chuyển từ ${sourceTable.name}`;
    const updatedNote = sourceTable.note 
      ? `${sourceTable.note}; ${transferText}` 
      : transferText;

    await updateDoc(doc(db, 'tables', targetTableId), {
      status: 'occupied',
      orders: sourceTable.orders || [],
      startTime: sourceTable.startTime || new Date().toISOString(),
      note: updatedNote
    });
  ...
  const handleCheckout = async (tableId: string, ordersToPay: OrderSession[], discount: number, paymentMethod: 'cash' | 'transfer', shouldPrint: boolean = false, customerId?: string) => {
    ...
    // Retrieve table from active state or fallback to getDoc
    let table = tables.find(t => t.id === tableId) || selectedTable;
    if (!table || table.id !== tableId) {
      const tableDoc = await getDoc(doc(db, 'tables', tableId));
      if (!tableDoc.exists()) return;
      table = { ...tableDoc.data(), id: tableDoc.id } as Table;
    }
    ...
    // Combine notes from orders being paid and table note (for merged/transferred tables)
    const validNotes = ordersToPay
      .map(o => o.note?.trim())
      .filter((n): n is string => Boolean(n));
    const combinedNotes = validNotes.join('; ');

    const noteParts: string[] = [];
    if (table.note && table.note.trim()) {
      noteParts.push(table.note.trim());
    }
    if (combinedNotes) {
      noteParts.push(combinedNotes);
    }
    const finalInvoiceNote = noteParts.join(' | ');

    // Create Invoice
    const allItems = ordersToPay.reduce((acc, o) => acc.concat(o.items), [] as OrderItem[]);
    
    // Group items by ID, name, price, and add-ons
    ...
    // Setup Atomic Firestore Batch Write for lightning-fast execution
    ...
    batch.set(invoiceRef, { ...invoice, storeId: currentUser.storeId });

    if (customerId) {
      const targetCustomer = customers.find(c => c.id === customerId);
      const currentPoints = targetCustomer?.points || 0;
    ...
    const tableUpdateData = {
      ...
      note: remainingOrders.length === 0 ? deleteField() : (table.note || deleteField())
    };

    batch.update(doc(db, 'tables', tableId), tableUpdateData);
    ...
    for (const item of invoice.items) {
      // 1. Direct goods deduction
      if (item.type === 'goods') {
        const menuItem = menu.find(m => m.id === item.id);
        if (menuItem && menuItem.trackStock !== false) {
           const currentStock = stockUpdates[item.id] !== undefined ? stockUpdates[item.id] : menuItem.stock;
           const newStock = Number((currentStock - item.quantity).toFixed(2));
           stockUpdates[item.id] = newStock;
           batch.update(doc(db, 'menu', item.id), { stock: newStock });
           
           // Record Stock Card
           const cardRef = doc(collection(db, 'stock_card'));
           const cardEntry: StockCardEntry = {
             itemId: menuItem.id,
             ...
             remaining: newStock,
             note: `Bán hàng: ${invoice.id || table.name}`
           };
           batch.set(cardRef, { ...cardEntry, storeId: currentUser.storeId });
        }
      } 
      // 2. Dish recipe ingredients deduction
        ...
        for (const recipeItem of item.recipe) {
          ...
          if (ingredient && ingredient.trackStock !== false) {
            const deduction = Number((recipeItem.quantity * item.quantity).toFixed(2));
            const currentStock = stockUpdates[ingredient.id] !== undefined ? stockUpdates[ingredient.id] : ingredient.stock;
            const newStock = Number((currentStock - deduction).toFixed(2));
            stockUpdates[ingredient.id] = newStock;
            batch.update(doc(db, 'menu', ingredient.id), { stock: newStock });
      ...
      // 3. Side dishes / Add-ons deduction from inventory
      if (item.selectedAddOns && item.selectedAddOns.length > 0) {
        for (const addon of item.selectedAddOns) {
          ...
          if (invItem && invItem.trackStock !== false) {
            ...
            const deduction = Number((addOnQty * item.quantity).toFixed(2));
            const currentStock = stockUpdates[invItem.id] !== undefined ? stockUpdates[invItem.id] : invItem.stock;
            const newStock = Number((currentStock - deduction).toFixed(2));
            stockUpdates[invItem.id] = newStock;
            batch.update(doc(db, 'menu', invItem.id), { stock: newStock });

            // Record Stock Card for add-on
            const cardRef = doc(collection(db, 'stock_card'));
            const cardEntry: StockCardEntry = {
              itemId: invItem.id,
              ...
              change: -deduction,
              remaining: newStock,
              note: `Món kèm [${addon.name}] của [${item.name}] x${item.quantity}`
            };
            batch.set(cardRef, { ...cardEntry, storeId: currentUser.storeId });
          }
        }
    ...
    if (activeShift) {
      ...
      batch.update(doc(db, 'shifts', activeShift.id), { 
    ...
    // Execute atomic batch commit - 1 single round-trip instead of 20+ sequential awaits!
    await batch.commit();

    // Optimistic local state updates for buttery-smooth UX
    if (Object.keys(stockUpdates).length > 0) {
      setMenu(prev => prev.map(m => stockUpdates[m.id] !== undefined ? { ...m, stock: stockUpdates[m.id] } : m));
    }
    ...
    setTables(prev => prev.map(t => t.id === tableId ? {
      ...
      currentOrder: [],
      discount: 0,
      startTime: remainingOrders.length === 0 ? undefined : (newStartTime || t.startTime),
      note: remainingOrders.length === 0 ? undefined : t.note
    } : t));
  ...
  const handleUpdateSettings = async (settings: SystemSettings) => {
    ...
    // Update Store Name (admin/manager only)
    if (settings.storeName && (currentUser.role === 'admin' || currentUser.role === 'manager' || currentUser.role === 'superadmin')) {
      try {
    ...
    // Update Admin Account if changed
    if (currentUser.role === 'admin' || currentUser.role === 'superadmin') {
      const updates: any = {};
      if (settings.adminUsername && settings.adminUsername !== currentUser.username) {
        updates.username = settings.adminUsername;
      }
      if (settings.adminPassword) {
        updates.password = settings.adminPassword;
      }
      if (Object.keys(updates).length > 0) {
        try {
          ...
          safeStorage.setItem('fnb_master_user', JSON.stringify(updatedUser));
        } catch (err) {
    ...
    try {
      ...
      console.error('Error updating settings collection:', err);
    }
  };
  ...
  const handleCheckIn = async (record: Partial<AttendanceRecord>) => {
    try {
      ...
      if (latestRecord && latestRecord.checkInTime && !latestRecord.checkOutTime) {
        ...
        return { type: 'check_out', time: nowIso };
      } else {
        // Must check-in
        ...
        return { type: 'check_in', time: nowIso };
      }
    } catch (e) {
  ...
  const attendanceStatus: 'none' | 'checked_in' | 'checked_out' = !latestTodayRecord 
    ? 'none' 
    : (latestTodayRecord.checkOutTime ? 'checked_out' : 'checked_in');
                      ...
                      <tr key={idx} className="border-b border-gray-100">
                        <td className="py-1">
                          ...
                          {item.selectedAddOns?.map(addon => (
                            <div key={addon.id} className="text-[10px] ml-2">+ {addon.name} {addon.price > 0 ? `(+${addon.price.toLocaleString()}đ)` : ''}</div>
                          ))}
                        </td>
    ...
    if (currentUser.store?.subscription?.validUntil) {
      ...
      if (daysLeft >= 0 && daysLeft <= 7) {
        allNotifications.push({
          ...
          description: `Thời hạn của tài khoản sẽ hết ${daysLeft === 0 ? 'hôm nay' : `sau ${daysLeft} ngày`}.`,
          type: 'warning'
        });
      }
      ...
      if (daysLeft >= 0 && daysLeft <= 7) {
        allNotifications.push({
          id: 'trial-expiring',
          title: 'Thời hạn dùng thử',
          description: `Thời hạn dùng thử sẽ kết thúc ${daysLeft === 0 ? 'hôm nay' : `sau ${daysLeft} ngày`}.`,
          type: 'warning'
        });
      }
    }
